...
, ST.st_dept, SC.s_subject, SC.s_score
FROM tbl_student ST
	JOIN tbl_score SC
		ON ST.st_num = SC.s_stnum;
...
MYSQL 수학함수
ROUND() : 실수(float)를 소수점 값 반올림하여 값 정리하기
ROUND(값, 소수자리수)
ex) ROUND(3.62) : 결과는 4, ROUND(3.62,1) : 결과는 3.6
TRUNCATE() : 실수(float) 를 버리기 하여 값 정리하기
TRUNCATE(3.62,0) : 결과는 3, TRUNCATE(3.62,1) : 결과는 3.6
*/
SELECT ST.st_num, ST.st_name, 
	SUM(IF(SC.s_subject = '국어', SC.s_score,0)) AS 국어,
    SUM(IF(SC.s_subject = '영어', SC.s_score,0)) AS 영어,
...
전체 데이터를 GROUP BY 하고 
국어점수, 총점, 평균을 계산한 후 
HAVING 절을 통하여 조건을 주어 원하는 결과를 찾는 것보다
효율적인 연산이 된다.
*/
 SELECT ST.st_num, ST.st_name, 
	SUM(IF(SC.s_subject = '국어', SC.s_score,0)) AS 국어,
...
SELECT SUM(s_score) AS 총점,
	ROUND(AVG(s_score),2) AS 평균
	FROM tbl_score
	WHERE s_subject = '국어';
...
-- PIVOT 테이블로 생성
SELECT 
SUM(IF(s_subject = '국어', s_score,null)) AS 국어총점,
