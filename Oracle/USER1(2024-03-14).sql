...
문자열 : CHAR(고정문자열), VARCHAR(가변문자열)
    ...
    CHAR 타입을 사용하려면 반드시 알파벳 문자가 있는 형태로
    설계를 하는 것이 좋다
    
    그래서 문자열의 경우 VARCHAR 를 사용해도 큰 문제가 없으므로
    고정문자열로 VARCHAR 로 사용하자
    
    CHAR : 최대 영숫자 2000자 까지
    VARCHAR : 최대 영숫자 4000자 까지
    
    오라클 최근 버전에서는 VARCHAR , nVARCHAR 대신 VARCHAR2 , nVARCHAR2 를 사용한다.
...
오라클에서 AUTO INCREMENT 를 실행하기 위해서는 
SEQUENCE 라는 객체를 생성하여 사용한다.
