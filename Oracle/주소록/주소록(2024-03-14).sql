...
SELECT a_hobby1 as 취미 , count(a_hobby1) as 사람수
FROM tbl_address
GROUP BY a_hobby1
ORDER BY 사람수 DESC;
...
-- UNION 은 다른것들 다 포함해줌 단 , 칼럼 숫자가 같아야함
SELECT * FROM
(
...
SELECT * FROM
(
SELECT a_code AS 코드,a_hobby1 AS 취미 FROM tbl_address WHERE a_hobby1 IS NOT NULL GROUP BY a_hobby1,a_code
UNION
SELECT a_code,a_hobby2 FROM tbl_address WHERE a_hobby2 IS NOT NULL GROUP BY a_hobby2,a_code
UNION
SELECT a_code,a_hobby3 FROM tbl_address WHERE a_hobby3 IS NOT NULL GROUP BY a_hobby3,a_code
)
ORDER BY 코드;
...
-- 두개의 Table 이 1:N 의 관계일때 N 의 Table 에서 설정한다.

-- Relation 과 tbl_address 간의 관계 설정
ALTER TABLE tbl_addr_hobby
ADD CONSTRAINT fk_addr 
FOREIGN KEY (r_acode)
REFERENCES tbl_address(a_code);

-- Relation 과 tbl_hobby 간의 관계 설정
...
산술 통계를 할때 거의 필수적으로 포함되는 GROUP BY 절이 필요하다.
*/
...
SELECT r_hcode,h_name AS 취미명 , COUNT(r_hcode)
FROM view_address
GROUP BY r_hcode ,h_name
ORDER BY h_name;

-- 문자열 칼럼의 데이터의 범위를 지정하여 확인하기
...
SELECT r_hcode,h_name AS 취미명 , COUNT(*) AS 숫자
FROM view_address
