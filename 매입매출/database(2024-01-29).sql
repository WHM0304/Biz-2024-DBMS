...
SELECT 
IF(io_div= '1','매입','매출') 구분,
SUM(io_quan * io_iprice) 매입합계,
SUM(io_quan * io_oprice) 매출합계
FROM tbl_iolist
GROUP BY io_div;
...
RDBMS(관계형데이터베이스 관리 시스템) 에서
	...
	데이터의 중복을 최소화하고, 데이터를 구조화 하는 것
    
정규화 목표 : 이상(문제점)이 있는 관계를 재구성 하고
	잘 조직된 관계를 생성하는 것이다
    
데이터의 이상(문제점보다는 다소 약한)현상
삽입(추가)이상
...
수정, 삭제 이상은 PK 가 아닌 다른 칼럼을 조건으로
	하여 실행하는 과정에서 발생한다.
목적 :
	이러한 이상 현상을 방지하기 위하여 정규화를 실행한다.
*/

-- 매입매출 Table 에서 상품정보와 거래처정보를
-- 분리하여 Table 을 구조화 하는 것

-- 상품정보를 Table로 별도 분리하기
-- 1. 매입매출 Table 에서 상품명을 중복없이 추출하기
SELECT io_pname,
ROUND(AVG(io_iprice),0) 매입단가,
ROUND(AVG(io_oprice),0) 매출단가
FROM tbl_iolist
GROUP BY io_pname
ORDER BY io_pname;

-- 2. 매입매출 테이블의 상품명 칼럼 대신, 상품코드 칼럼으로 대체
-- 기존의 테이블을 삭제하고, 새로 작성
...
-- tbl_iolist 와 tbl_products 테이블을 JOIN 하여
-- Projection 하기
