...
다른 DB에서 사용하거나
DB를 새로 설치할 경우 사전에 기존의 사용하던 DB를 백업하기
...
참조 무결성이 되지 않은 Table 을 EQ JOIN 으로 연계하는경우
보여지는 데이터가 누락되거나, 전혀 엉뚱한 리스트가
보여질수 있다

이때는 EQ JOIN 이 아닌 OUTER JOIN 이나 INNER JOIN을
사용해야 한다
INNER JOIN은 실무에서 별로 사용하지 않고
주로 OUTER JOIN을 사용한다
OUTER JOIN에는 LEFT OUT JOIN, RIGHT OUT JOIN 이 있으며
...
요즘의 DB 소프트웨어 에서는 VIEW 를 통하여 제한적으로 CRUD 가 
모두 가능하다
...
SELECT io_pcode,p_name AS 상품명,
SUM(io_quan * io_iprice) AS 매입금액합산,
SUM(io_quan * io_oprice) AS 매출금액합산,
SUM( io_quan * io_oprice - io_quan *io_iprice ) AS 이익금
FROM view_iolist
GROUP BY p_name, io_pcode
...
-- 매입매출 데이터에서 매입항목(io_div = 1)만 SELECTION 하여
-- 상품코드 ,상품명, 매입합계를 출력
